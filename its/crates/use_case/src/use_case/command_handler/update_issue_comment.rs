use domain::{aggregate::issue_comment::attribute::IssueCommentText, IssueCommentId};
...
pub async fn update_issue_comment<C: HasIssueCommentRepository + ?Sized>(
    context: &C,
    ...
    // pure
    let updated = issue_comment.update(text, at)?;

    // io
    context.issue_comment_repository().save(&updated).await?;

    let issue_comment_id = updated
        .events()
        .iter()
        .next()
        .map(|event| event.issue_comment_id().to_owned())
        .expect("invalid event seq");
    Ok(IssueManagementContextEvent::IssueCommentUpdated { issue_comment_id })
}
