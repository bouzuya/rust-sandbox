...
fn graph(s: &str) -> IResult<&str, Graph> {
    ...
    map_res(
        ...
        |(_strict, directed, name, _, statements, _)| {
            let g = if directed {
                Graph::digraph
            } else {
                Graph::graph
...
fn edge_stmt(s: &str) -> IResult<&str, Statement> {
    ...
    map(
        ...
        |(l, (d, r), a)| Statement::Edge(d, l, r, a.unwrap_or_default()),
    )(s)
}

fn edge_rhs(s: &str) -> IResult<&str, (bool, Either<String, Graph>)> {
...
fn edgeop(s: &str) -> IResult<&str, bool> {
    map(alt((tag("->"), tag("--"))), |t| t == "->")(s)
}
...
mod tests {
    ...
    fn es(d: bool, l: &str, r: &str) -> Statement {
        Statement::Edge(
            d,
    ...
    fn eswa(d: bool, l: &str, r: &str, attr_list: AttrList) -> Statement {
        Statement::Edge(
            d,
            Either::Left(l.to_string()),
            Either::Left(r.to_string()),
    ...
    fn graph_test() -> anyhow::Result<()> {
        ...
        assert_eq!(graph("graph {}"), Ok(("", Graph::graph(None, vec![])?)));
        assert_eq!(graph("graph{}"), Ok(("", Graph::graph(None, vec![])?)));
        assert_eq!(
            graph("graph {n}"),
            Ok((
                "",
                Graph::graph(None, vec![Statement::Node("n".to_string(), vec![])])?
            ))
        );
        assert_eq!(
        ...
        assert_eq!(
            ...
            Ok(("", Graph::digraph(Some("example".to_string()), vec![])?))
        );
        assert_eq!(
            graph(r#"digraph "example graph" {}"#),
            Ok((
                "",
                Graph::digraph(Some("example graph".to_string()), vec![])?
            ))
        );
        assert_eq!(
        ...
        // direction
        assert_eq!(graph(r#"digraph { N1 -- N2 }"#).is_err(), true);
        assert_eq!(graph(r#"digraph { { N1 -- N2 } }"#).is_err(), true);
        assert_eq!(graph(r#"digraph { { { N1 -- N2 } } }"#).is_err(), true);
        assert_eq!(graph(r#"digraph { {} }"#).is_err(), false);
        assert_eq!(graph(r#"graph { N1 -> N2 }"#).is_err(), true);
        assert_eq!(graph(r#"graph { { N1 -> N2 } }"#).is_err(), true);
        assert_eq!(graph(r#"graph { { { N1 -> N2 } } }"#).is_err(), true);
        assert_eq!(graph(r#"graph { {} }"#).is_err(), false);
        Ok(())
    }
    ...
    fn stmt_list_test() {
        ...
        assert_eq!(f("N1 -> N2"), Ok(("", vec![es(true, "N1", "N2")])));
        assert_eq!(f("N1 -- N2"), Ok(("", vec![es(false, "N1", "N2")])));
        ...
        assert_eq!(
            f("N1 N2 -> N3 N4"),
            Ok(("", vec![ns("N1"), es(true, "N2", "N3"), ns("N4"),]))
        );
    ...
    fn stmt_test() -> anyhow::Result<()> {
        ...
        assert_eq!(stmt("N1 -> N2"), Ok(("", es(true, "N1", "N2"))));
        assert_eq!(stmt("N1 -- N2"), Ok(("", es(false, "N1", "N2"))));
        assert_eq!(
            stmt("node [N1=V1]"),
            Ok((
                "",
    ...
    #[test]
    fn edge_stmt_test() {
        assert_eq!(edge_stmt("N1 -> N2"), Ok(("", es(true, "N1", "N2"))));
        assert_eq!(edge_stmt("N1 -- N2"), Ok(("", es(false, "N1", "N2"))));
        assert_eq!(
            edge_stmt("N1 -- N2 [K1=V1]"),
            Ok(("", eswa(false, "N1", "N2", al(&[("K1", "V1")]))))
        );
    }
    ...
    #[test]
    fn subgraph_test() -> anyhow::Result<()> {
        assert_eq!(
