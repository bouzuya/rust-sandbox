...
fn main() {
    ...
    match args[1].as_str() {
        ...
        "r" => {
            ...
            let mut vm = Vm::new(Rc::new(byte_code));
...
impl Vm {
    ...
    fn init_fn(&mut self, name: &str, args: &[Value]) -> Result<(), Box<dyn std::error::Error>> {
        let fn_def = self
            .bytecode
            .funcs
            .get(name)
        ...
        loop {
            ...
            match instruction.op {
                ...
                OpCode::Call => {
                    ...
                    let fn_def =
                        self.bytecode.funcs.get(&name).ok_or_else(|| {
                            format!("Function name shall be a string: {:?}", name)
                        })?;
                    match fn_def {
                        FnDef::User(f) => {
                            if f.cofn {
                                ...
                                vm.stack_frames
                                    .push(StackFrame::new(f.clone(), args.to_vec()));
                                let stack = &mut self.top_mut()?.stack;
                                stack.resize(
    ...
    fn return_fn(
        ...
        let stack = &mut self.top_mut()?.stack;
        stack.resize(stack.len() - args - 1, Value::F64(0.0));
        stack.push(res);
        self.top_mut()?.ip += 1;
        Ok(None)
    }
    ...
    #[allow(dead_code)]
    fn run_fn(&mut self, name: &str, args: &[Value]) -> Result<Value, Box<dyn std::error::Error>> {
        let fn_def = self
            .bytecode
            .funcs
            .get(name)
        ...
        match self.interpret()? {
            ...
            YieldResult::Suspend(_) => Err("Yeilded at toplevel".into()),
        }
    }
    ...
    fn top(&self) -> Result<&StackFrame, String> {
        self.stack_frames
            .last()
            .ok_or_else(|| "Stack frame underflow".to_owned())
    }
...
impl std::fmt::Display for Value {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Value::Coro(_) => write!(f, "<Coroutine>"),
        }
    }
}
...
impl ByteCode {
    ...
    fn read_funcs(&mut self, reader: &mut impl std::io::Read) -> std::io::Result<()> {
        ...
        let mut funcs = standard_functions()
            .into_iter()
            .filter_map(|(name, f)| {
...
impl Compiler {
    ...
    fn compile(&mut self, stmts: &Statements) -> Result<(), Box<dyn std::error::Error>> {
        ...
        self.add_fn("main".to_owned(), &[], false);
        Ok(())
    }
...
impl From<u8> for OpCode {
    fn from(o: u8) -> Self {
        ...
        const YIELD: u8 = OpCode::Yield as u8;
        const AWAIT: u8 = OpCode::Await as u8;
        match o {
...
fn await_expr(i: Span) -> IResult<Span, Expression> {
    ...
    let (i, expr) = cut(space_delimited(expr))(i)?;
    Ok((
        i,
        Expression::new(ExprEnum::Await(Box::new(expr)), calc_offset(i0, i)),
    ))
}
...
fn fn_def_statement(i: Span) -> IResult<Span, Statement> {
    let (i, fn_kw) = space_delimited(alt((tag("fn"), tag("cofn"))))(i)?;
...
mod tests {
    ...
    fn test_statements_finish() {
        ...
        .is_ok());
        assert!(
            statements_finish(Span::new("print(123); yield 123; print(456); yield 321;")).is_ok()
        );
    }
}
